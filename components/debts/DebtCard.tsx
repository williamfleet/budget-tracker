...
export default function DebtCard({ debt, onEdit }: DebtCardProps) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-indigo-500">
      ...
      {debt.notes && (
        <p className="text-xs text-gray-600 mb-4 italic">{debt.notes}</p>
      )}
      ...
      <div className="flex gap-2">
        <button
          onClick={() => onEdit(debt)}
          className="flex-1 px-3 py-2 text-sm bg-indigo-50 text-indigo-700 rounded-md hover:bg-indigo-100 font-medium"
        >
          Edit
        </button>
        {debt.is_active && debt.balance > 0 && (
          <button
            ...
            className="flex-1 px-3 py-2 text-sm bg-green-50 text-green-700 rounded-md hover:bg-green-100 font-medium"
          >
            Mark Paid Off
          </button>
        )}
        <button
          ...
          disabled={isDeleting}
          className="px-3 py-2 text-sm bg-red-50 text-red-700 rounded-md hover:bg-red-100 font-medium disabled:opacity-50"
        >
          {isDeleting ? '...' : 'Delete'}
        </button>
      </div>
